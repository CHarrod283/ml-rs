...
use matrix::{MatrixStats, MatrixMath};
use matrix::Matrix;
use rand::Rng;
use ndarray::prelude::*;

trait Layer {
    fn forward_propagation(&self, input_data: &Matrix, output: &mut Matrix) -> Result<(), String> ;
    fn backward_propagation(&mut self, derivative_error_wrt_output: &mut Matrix, input: &mut Matrix, learning_rate: f64) -> Result<(), String> ;
}
...
impl FCLayer {
    fn new(input_size : usize, output_size : usize) -> Result<FCLayer, String> {
        Ok(FCLayer {
...
impl Layer for FCLayer {
    fn forward_propagation(&self, input_data: &Matrix, output :&mut Matrix) -> Result<(), String> {
        Matrix::target_dot(output, input_data, &self.weights)?;
        Matrix::inline_add(output, &self.bias)?;
        Ok(())
    }


    fn backward_propagation(&mut self, output_error: &mut Matrix, input_data: &mut Matrix, learning_rate: f64) -> Result<(), String> {
...
//
        ...
        input_data.inline_transpose();
        Matrix::target_dot(&mut self.weights_error, input_data, output_error)?;
        input_data.inline_transpose();

        self.weights.inline_transpose();
        Matrix::target_dot(input_data, output_error, &self.weights)?;
        self.weights.inline_transpose();

        Matrix::inline_scalar_mult(&mut self.weights_error, learning_rate);
        Matrix::inline_sub(&mut self.weights, &self.weights_error)?;

        Matrix::inline_scalar_mult(output_error, learning_rate);
        Matrix::inline_sub(&mut self.bias, output_error)?;
        Ok(())
    }
}
...
impl Layer for ActivationLayer {
    fn forward_propagation(&self, input_data: &Matrix, output :&mut Matrix) -> Result<(), String> {
        Matrix::target_apply(output, input_data, self.activation)?;
        Ok(())
    }


    fn backward_propagation(&mut self, output_error: &mut Matrix, input_data: &mut Matrix, _learning_rate: f64) -> Result<(), String> {
        Matrix::inline_apply(input_data, self.activation_prime);
        Matrix::inline_mult(input_data, output_error)?;
        Ok(())
    }
}
...
fn mse(y_true :&Matrix, y_pred : &Matrix) -> f64 {
    ...
    Matrix::inline_sub(&mut arr, y_pred).unwrap();
    Matrix::inline_apply(&mut arr, power2);
    arr.mean()
}
...
fn mse_prime(y_true :&Matrix, y_pred : &Matrix) -> Matrix {
    let s = y_true.size();
    let size = s.1 * s.0;
    let mul = 2_f64 / size as f64;
    let mut arr = y_pred.clone();
    Matrix::inline_sub(&mut arr, y_true).unwrap();
    Matrix::inline_scalar_mult(&mut arr, mul);
    arr
}
...
impl Network {
    ...
    fn predict_individual(&mut self, input_data : &Matrix) -> Result<Matrix, String> {
    ...
    fn predict(&mut self, input_data : &[Matrix]) -> Result<Vec<Matrix>, String> {
